...
static RE_GIT_LS_REMOTE: Lazy<Regex> =
    Lazy::new(|| Regex::new("^(?P<hash>\\w*)\\s*refs/pull/(?P<prid>\\d+)/head$").unwrap());
static RE_BODY_TASK_LIST_CHECKED: Lazy<Regex> =
    Lazy::new(|| Regex::new("-\\s\\[x\\]\\s#").unwrap());
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    if !list_pr.is_empty() {
        ...
        if let Some(html_url) = &list_pr[0].html_url {
            println!(
                "existing PullRequest was successfully updated: {}",
                html_url.as_str()
            );
        }
    } else {
        ...
        if let Some(html_url) = &ret.html_url {
            println!(
                "new PullRequest was successfully created: {}",
                html_url.as_str()
            );
        }
